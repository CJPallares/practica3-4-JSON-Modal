...
export class ApiServiceProvider {
  ...
  modificarAlumno(
    ...
    let promise = new Promise<Alumno>((resolve, reject) => {
      ...
      this.http
        .put(this.URL + '/alumnos/' + idAlumno, JSON.stringify(datos), header)
        .toPromise()
        .then((data: any) => {
          // Success
          let alumno: Alumno;
  ...
  insertarAlumno(datosNuevoAlumno: Alumno): Promise<Alumno> {
    let promise = new Promise<Alumno>((resolve, reject) => {
      ...
      delete datos.id; //OJO! cuando se hace un post no paso el id. El id es asignado por el servidor. Quito el atributo del objeto json
      this.http
        .post(this.URL + '/alumnos/', JSON.stringify(datos), header)
        .toPromise()
        .then((data: any) => {
          // Success
          let alumno: Alumno;
